...
phpstan: ## Static analysis via isolated tools/phpstan (src + config + tests)
...
behat: ## Run the Behat feature suite
	$(DOCKER_COMPOSE) exec app composer behat
...
fix-permissions: ## Change permission for volume in the app container
...
migrate: ## Run migrations
	$(DOCKER_COMPOSE) exec app composer migrate
